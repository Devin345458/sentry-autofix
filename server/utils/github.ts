...
export async function createPullRequest(params: {
  ...
  const { repo, branch, baseBranch, parsed, changedFiles } = params
 ...
 */
export async function ensureLabel(repo: string): Promise<void> {
  try {
    await execFileAsync(
      'gh',
...
function buildPrBody(parsed: ParsedEvent, changedFiles: string[]): string {
  ...
  body += `\n---\n`
  body += `*Generated by sentry-autofix using Claude Code*\n`
